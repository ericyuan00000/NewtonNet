...
class Trainer(object):
    ...
    def train(self):       
        ...
        log_one_epoch = log_one_epoch | {f'final_train_{key}': value for key, value in train_log.items()}
        if self.val_generator is not None:
            val_log = self.run_one_epoch(self.val_generator, step=False)
            log_one_epoch = log_one_epoch | {f'final_val_{key}': value for key, value in val_log.items()}
        if self.test_generator is not None:
            test_log = self.run_one_epoch(self.test_generator, step=False)
            log_one_epoch = log_one_epoch | {f'final_test_{key}': value for key, value in test_log.items()}
        if self.output_path is not None:
            self.local_log(log_one_epoch)
        if self.log_wandb:
            wandb.log(log_one_epoch)
